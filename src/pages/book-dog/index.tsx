...
import { DOG_ADDONS } from '@/data/addons'
...
const BookDogPage: React.FC = () => {
  ...
  const loadPets = async () => {
    try {
      ...
      if (list && list.length > 0) {
        const firstDog = list.find(p => p.type === 'dog')
        if (firstDog && selectedPets.length === 0) {
          setSelectedPets([firstDog.id])
        }
      }
    } catch (err) {
      console.error('[BookDog] 加载宠物失败', err)
    }
  }
  ...
  const timeOptions = ['07:00', '09:00', '11:00', '14:00', '16:00', '18:00', '19:30']
  ...
  const selectedAddonList: AddOnItem[] = useMemo(() => {
    return selectedAddons.map(id => DOG_ADDONS.find(a => a.id === id)).filter(Boolean) as AddOnItem[]
  }, [selectedAddons])

  const [priceResult, setPriceResult] = useState<PriceResult>({
    basePrice: 25, distanceFee: 0, petCountFee: 0, addOnsFee: 0, holidayFee: 0,
    totalPrice: 25, details: [{ label: '上门遛狗 基础价', amount: 25 }]
  })
  ...
  const updatePrice = async () => {
    try {
      const petsForCalc = selectedPetList.length > 0
        ? selectedPetList
        : [{ id: 'temp', name: '', type: 'dog', breed: '', personality: '', dogSize: 'small' }] as Pet[]
      ...
      console.warn('[BookDog] 价格计算失败', err)
    }
  }
  ...
  useEffect(() => { if (selectedPets.length > 0) updatePrice() }, [selectedPets.join(',')])
  ...
  const handleSubmit = async () => {
    if (!selectedDate) { Taro.showToast({ title: '请选择服务日期', icon: 'none' }); return }
    if (!selectedTime) { Taro.showToast({ title: '请选择服务时间', icon: 'none' }); return }
    if (selectedPets.length === 0) { Taro.showToast({ title: '请选择宠物', icon: 'none' }); return }
    if (!address) { Taro.showToast({ title: '请填写服务地址', icon: 'none' }); return }
    ...
    try {
      ...
      console.error('[BookDog] 下单失败', err)
  ...
  return (
    <View className={styles.page}>
      <View className={styles.serviceHeader}>
        <Text className={styles.headerTitle}>🐶 上门遛狗</Text>
        <Text className={styles.headerDesc}>遛狗30分钟·喂食·擦脚，基础价25元/次</Text>
      </View>
      ...
      <View className={styles.section}>
        <Text className={styles.sectionTitle}>🐾 选择服务的狗狗</Text>
        {dogs.length === 0 && (
          <Text style={{ fontSize: '28rpx', color: '#A8A29B' }}>暂无狗狗，请先在「我的-我的宠物」添加</Text>
        )}
              ...
              >
                ...
                <View className={styles.petInfo}>
                  ...
                  <Text className={styles.petBreed}>
                    {pet.breed} · {pet.weight}kg · {pet.personality}
                  </Text>
                </View>
